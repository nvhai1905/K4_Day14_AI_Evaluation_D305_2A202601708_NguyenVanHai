...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) >= 2:
            first_avg = avg_score(scores_batch[0])
            rest_avgs = [avg_score(entry) for entry in scores_batch[1:]]
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def avg(results_list: list, attr: str) -> float:
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        low_scores = [name for name, value in scores.items() if value < threshold]

        if len(low_scores) > 1:
            return "Multiple issues detected — review full pipeline"

        weakest = low_scores[0] if low_scores else min(scores, key=scores.get)
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            ...
            "irrelevant": (
                "Add few-shot examples and tighten prompt instructions so answers "
                "directly address the question intent"
            ),
            "incomplete": (
                "Increase chunk size or top-k retrieval to reduce context "
                "fragmentation and improve completeness"
            ),
            "off_topic": (
                "Improve intent detection/routing before generation to keep "
                "answers on-topic"
            ),
            "refusal": (
                "Loosen overly strict guardrails for in-scope questions while "
                "keeping out-of-scope refusals"
            ),
        }

        categories = self.categorize_failures(failures)
        suggestions = [
            suggestion_map[category]
            for category in categories
            if category in suggestion_map
        ]

        fallback_suggestions = [
            "Expand the golden dataset with these failure patterns to prevent regressions",
            "Review retrieval and generation traces manually for the lowest-scoring cases",
            "Re-run regression testing after each fix to confirm the metric recovers",
        ]
